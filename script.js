...
function MySprite(img_url) {
  ...
  function ImagesTouching(thing1, thing2){
    ...
    if (
        thing1.x >=thing2.x + thing2.MyImg.width || thing1.x + thing1.MyImg.width <= thing2.x
    ) return false;
    if (
...
function make_bird_slow_and_fall(){
    if (bird.velocity_y < max_fall_speed){
        bird.velocity_y = bird.velocity_y+accel;
    }
...
function display_bar_running_along_bottom(){
    ...
    ctx.drawImage(bottom_bar, bottom_bar_offset,
...
function reset_game(){
    ...
    add_all_my_pipes(); // load all the pipes back in their starting positions
}
